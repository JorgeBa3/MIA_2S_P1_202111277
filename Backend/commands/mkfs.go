...
func ParserMkfs(tokens []string) (string, error) {
	...
	resultado := fmt.Sprintf("Comando mkfs ejecutado en la partición %s", cmd.id)
	return resultado, nil // Devuelve el comando MKFS creado
}
...
func calculateN(partition *structures.PARTITION) int32 {
	...
	*/
	...
	numerator := int(partition.Part_s) - binary.Size(structures.SuperBlock{})
